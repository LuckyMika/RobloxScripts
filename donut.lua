...
local function getDonut()
    for i, v in next, {"Backpack", "Character"} do
        for i2, v2 in next, Plr[v]:GetChildren() do
            if v2:IsA("Tool") and string.find(v2.Name, "Donut") then
                return v2
            end
        end
    end
...
local function autoSell()
    firetouchinterest(Root, workspace.SellRing.Trigger, 0)
    firetouchinterest(Root, workspace.SellRing.Trigger, 1)
end

local function getHighestDonutName()
    local getHighestOwnedPrice = 0
    for i, v in next, Plr.DonutsFolder:GetChildren() do
        if v:IsA("BoolValue") and v.Value then
            local storageDonut = ReSt.Donuts:FindFirstChild(v.Name)
            if storageDonut then
                local donutConfig = require(storageDonut.Config)

                if donutConfig.Price > getHighestOwnedPrice then
                    getHighestOwnedPrice = donutConfig.Price
                end
            end
        end
    end
    
    local donutName, lowestPrice = nil, getHighestOwnedPrice
    for i, v in next, ReSt.Donuts:GetChildren() do
        if v:FindFirstChild("Config") and Plr.DonutsFolder:FindFirstChild(v.Name) then
            
            local donutConfig = require(v.Config)
            if donutConfig.Price > lowestPrice and donutConfig.Price <= Plr.leaderstats.Money.Value then
                donutName, lowestPrice = v.Name, donutConfig.Price
            end
...
services.UserInputService.InputBegan:Connect(function (key, chat)
...
task.spawn(function ()
    while task.wait() do
        if flying and flyenabled then
            ...
            if w_pressed then
                Char.PrimaryPart:FindFirstChild("FlightVelocity").Velocity = cam.CFrame.LookVector * Vector3.new(speed,0,speed)
            end
            if a_pressed then
                Char.PrimaryPart:FindFirstChild("FlightVelocity").Velocity = cam.CFrame.RightVector * -1 * speed
            end
            if s_pressed then
                Char.PrimaryPart:FindFirstChild("FlightVelocity").Velocity = cam.CFrame.LookVector * Vector3.new(-1 * speed,0,-1 * speed)
            end
            if d_pressed then
                Char.PrimaryPart:FindFirstChild("FlightVelocity").Velocity = cam.CFrame.RightVector * speed
            end
            if space_pressed then
                Char.PrimaryPart:FindFirstChild("FlightVelocity").Velocity = Vector3.new(0,speed,0)
            end
            if shift_pressed then
                Char.PrimaryPart:FindFirstChild("FlightVelocity").Velocity = Vector3.new(0,-1 * speed,0)
            end
...
task.spawn(function()
    while true do
        if autoFarmEnabled then
            local donutTool = getDonut()
            if donutTool then
                Hum:EquipTool(donutTool)
                donutTool:Activate()
            end
...
local donutsAmount = Plr.PlayerGui.Main.Frame.Donuts.Number
donutsAmount:GetPropertyChangedSignal("Text"):Connect(function()
    if tonumber(donutsAmount.Text) > 0 and autoSellEnabled then
        autoSell()
    end
end)

Plr.leaderstats.Money:GetPropertyChangedSignal("Value"):Connect(function()
    local highestDonutName = getHighestDonutName()
    if highestDonutName then

        for _ = 1, 2 do -- Buy + Equip
            ReSt.DonutShopEvent:FireServer(highestDonutName)
        end
...
local ui = library:Init('Donut Reaper X')

local farmtab = ui:Tab('Farming')
local misctab = ui:Tab('Misc')

local farmingsection = farmtab:Section('Farming')

farmingsection:Toggle('Auto Farm', 'autofarm', false, function (state)
    autoFarmEnabled = state
end)
farmingsection:Toggle('Auto Sell', 'autosell', false, function (state)
...
misc:Slider('WalkSpeed', 'ws', 16, 0, 500, function(value) -- string <text>, string <flag>, int <default>, int <min>, ini <max>, function [callback]
    game.Players.LocalPlayer.Character.Humanoid.WalkSpeed = value
end)

misc:Slider('JumpPower', 'jp', 50, 1, 1000, function(value) -- string <text>, string <flag>, int <default>, int <min>, ini <max>, function [callback]
    game.Players.LocalPlayer.Character.Humanoid.JumpPower = value
end)

misc:Toggle("Fly", "fly", false, function (state)
    flyenabled = state
...
misc:Slider("FlySpeed", "fs", 100, 0, 500, function (value)
    speed = value
end)

misc:Button('Destroy UI', library.destroy) -- string <text>, function [callback]
misc:Keybind('Toggle UI', 'toggleUi', Enum.KeyCode.RightShift, library.toggleui)

local credits = ui:Tab("Credits")

local creditssection = credits:Section("Credits")

creditssection:Label("UI Library: Old Vestra UI")
creditssection:Label("Scripting: Vynixu & Mika")
creditssection:Label("This is what happens when you advertise your game in our server")
creditssection:Button("https://discord.gg/vynixius", function ()
